...
class BusinessController extends OwnedCrudController {
 ...
 public function index(Request $r):JsonResponse { $q=$this->applyFilters($this->query($r),$r)->with(['links','socialLinks']);$order=$r->string('sort','display_order')->toString();if(!in_array($order,['display_order','created_at','updated_at'],true))$order='display_order';return response()->json($q->orderBy($order)->orderBy('created_at')->get()); }
